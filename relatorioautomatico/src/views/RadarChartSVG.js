...
export default function RadarChartSVG({ scores, size = 300, max = 5 }) {
  const center = size / 2;
  const radius = size / 2 - 40; // aumenta a margem para labels
  ...
  const labels = categories.map((cat, i) => {
    const angle = i * angleStep - Math.PI / 2;
    const x = center + (radius + 32) * Math.cos(angle); // aumenta a distância do label
    const y = center + (radius + 32) * Math.sin(angle) + 6; // pequeno ajuste vertical
    return (
      <SvgText
        key={cat}
        x={x}
